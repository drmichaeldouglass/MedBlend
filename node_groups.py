...
#initialize proton_spots node group
def proton_spots_node_group():
	...
	#Value_001
	math.inputs[1].default_value = 0.003000000026077032
	#Value_002
	math.inputs[2].default_value = 0.5
	...
	if "Spot_Source0" in bpy.data.objects:
		object_info.inputs[0].default_value = bpy.data.objects["Spot_Source0"]
	#As Instance
	...
	if "Beam" in bpy.data.materials:
		set_material_001.inputs[2].default_value = bpy.data.materials["Beam"]
	...
	#Set locations
	group_output.location = (1412.803955078125, 0.0)
	...
	set_material.location = (898.3999633789062, 25.270862579345703)
	join_geometry.location = (1193.1549072265625, 40.935890197753906)
	combine_xyz.location = (-242.89889526367188, -102.78641510009766)
	group_input.location = (-627.791015625, 35.1865348815918)
	mesh_to_points.location = (-137.73611450195312, 94.57881164550781)
	sample_index.location = (251.56118774414062, 325.8182067871094)
	object_info.location = (42.311866760253906, 441.63372802734375)
	curve_line.location = (460.1163024902344, 481.728271484375)
	curve_circle.location = (528.7009887695312, 256.3628845214844)
	curve_to_mesh.location = (735.8453369140625, 362.80499267578125)
	set_material_001.location = (973.1549072265625, 241.33462524414062)
	...
	proton_spots.links.new(named_attribute.outputs[0], combine_xyz.inputs[0])
	#named_attribute_001.Attribute -> combine_xyz.Y
	proton_spots.links.new(named_attribute_001.outputs[0], combine_xyz.inputs[1])
	#named_attribute_002.Attribute -> combine_xyz.Z
	proton_spots.links.new(named_attribute_002.outputs[0], combine_xyz.inputs[2])
	...
	proton_spots.links.new(named_attribute_003.outputs[0], math.inputs[0])
	...
	#integer.Integer -> sample_index.Index
	proton_spots.links.new(integer.outputs[0], sample_index.inputs[2])
	#combine_xyz.Vector -> sample_index.Value
	proton_spots.links.new(combine_xyz.outputs[0], sample_index.inputs[1])
	...
	proton_spots.links.new(sample_index.outputs[0], curve_line.inputs[1])
